...
EOF
    ...
    printf "#!/bin/sh\n/usr/bin/ssh -F $PROJECT_ROOT/.direnv/ssh_config \$@\n" > $TOOLS_PATH/ssh
...
if [ ! -e "${TOOLS_PATH}/terraform" ]; then
    ...
    TEMPDIR=$(mktemp -d)
    pushd $TEMPDIR &>> $LOG
    curl -sLo terraform.zip https://releases.hashicorp.com/terraform/1.3.10/terraform_1.3.10_linux_amd64.zip
...
if [ ! -e "${TOOLS_PATH}/terragrunt" ]; then
    ...
    curl -sLo ${TOOLS_PATH}/terragrunt https://github.com/gruntwork-io/terragrunt/releases/download/v0.63.2/terragrunt_linux_amd64
