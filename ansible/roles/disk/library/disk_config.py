...
module = AnsibleModule(argument_spec=dict(config=dict(required=True, type='list'),))
...
class nvme_identify_controller_amzn_vs(Structure):
    _pack_ = 1
    _fields_ = [("bdev", c_char * 32), ("reserved0", c_char * (1024 - 32))]  # block device name


class nvme_identify_controller_psd(Structure):
    _pack_ = 1
    _fields_ = [
        ("mp", c_uint16),  # maximum power
        ("reserved0", c_uint16),
...
def main():
    ...
    new_config = [update_disk(disk, mapping) for disk in src_config]
