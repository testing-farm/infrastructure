...
def _policy_prefer_openstack(logger: gluetool.log.ContextAdapter, pools: List[PoolDriver]) -> List[PoolDriver]:
...
def _policy_least_crowded(logger: gluetool.log.ContextAdapter, pools: List[PoolDriver]) -> List[PoolDriver]:
...
def _policy_enough_resources(
    ...
    suitable_pools = [pool for pool, metrics in pool_metrics if has_enough(pool, metrics)]
...
def hook_ROUTE(
    ...
    # NOTE: beaker should be added here later, if Artemis will properly support it
        ...
        pool_capabilities = [(pool, pool.capabilities()) for pool in pools]
