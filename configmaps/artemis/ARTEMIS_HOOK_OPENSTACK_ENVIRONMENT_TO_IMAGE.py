...
def _map_compose_to_name(logger: gluetool.log.ContextAdapter, compose_id: str) -> Result[str, Failure]:
    ...
    try:
        image_names = pattern_map.match(compose_id)

        if isinstance(image_names, list):
            return Ok(image_names[0])
...
def _image_by_name(logger: gluetool.log.ContextAdapter, pool: OpenStackDriver, image_name: str) -> Result[Any, Failure]:
...
def hook_OPENSTACK_ENVIRONMENT_TO_IMAGE(
    ...
    try:
        ...
        assert environment.os.compose is not None
